...
Object -> lamp "brass lamp"
  ...
  └─▶ ZILF front end   (ZIL → ZAP assembly)
        ...
        <ul class="text-muted list-disc space-y-1.5 pl-5 text-sm">
          ...
          <li>The compiler runs on the <span class="text-default">main thread</span> (a ZIL compile takes a few seconds; the UI shows a "Compiling…" state — an off-main-thread Web Worker is a documented follow-up, since the .NET runtime doesn't boot standalone in a Worker), and the <span class="text-default">~7.5&nbsp;MB gzipped</span> .NET bundle is <span class="text-default">lazy-loaded only on the first ZIL compile</span> — Inform 6 users never download it.</li>
